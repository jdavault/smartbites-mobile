...
export async function generateRecipes(
  ...
  try {
    ...
    const prompt = `
You are a professional culinary recipe writer.
Create exactly **5 different** recipes for "${query}".
${allergensText}
${dietText}
  ...
  "recipes": [
    {
      ...
      "nutritionInfo": "Nutrition per serving"
    }
  ]
}`;
    ...
    const recipes: GeneratedRecipe[] = Array.isArray(parsed.recipes) ? parsed.recipes : [];
...
export async function generateRecipeImage(title: string): Promise<string> {
  try {
    ...
    const data = await response.json();
    return data?.data?.[0]?.url || 'https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg';
  } catch (error) {
...
function generateMockRecipe(query: string, allergens: string[], dietaryPrefs: string[]): GeneratedRecipe {
  return {
